  ...
  def forward(self, x, w=1.0):
    ...
    if isscaler or w.size(0) == 1:
      ...
      return self._backend.Embedding.apply(
        x, weight, padding_idx, self.max_norm, self.norm_type, self.scale_grad_by_freq, self.sparse
      )
    else:
      ...
      for batch_idx, we in enumerate(w):
        ...
        pe.append(
          self._backend.Embedding.apply(
            x[batch_idx], weight, padding_idx, self.max_norm, self.norm_type, self.scale_grad_by_freq, self.sparse
          )
        )
      pe = torch.stack(pe)
      return pe
...
class Conv1dGLU(nn.Module):
  ...
  def __init__(
    ...
    **kwargs
    ...
    self.conv = Conv1d(
      in_channels, 2 * out_channels, kernel_size, dropout=dropout, padding=padding, dilation=dilation, *args, **kwargs
    )
    if n_speakers > 1:
