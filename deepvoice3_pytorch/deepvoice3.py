...
class Encoder(nn.Module):
  ...
  def __init__(
    ...
    self.convolutions.append(
      Conv1d(in_channels, embed_dim, kernel_size=1, padding=0, dilation=1, std_mul=std_mul, dropout=dropout)
    )
...
class Decoder(nn.Module):
  ...
  def __init__(
    ...
    self.embed_query_positions = SinusoidalEncoding(max_positions, convolutions[0][0], padding_idx)
    self.embed_keys_positions = SinusoidalEncoding(max_positions, embed_dim, padding_idx)
    ...
    self.last_conv = Conv1d(
      in_channels, in_dim * r, kernel_size=1, padding=0, dilation=1, std_mul=std_mul, dropout=dropout
    )
...
class Converter(nn.Module):
  ...
  def __init__(
    ...
    if time_upsampling == 4:
      self.convolutions = nn.ModuleList(
        [
          Conv1d(in_dim, in_channels, kernel_size=1, padding=0, dilation=1, std_mul=1.0),
          ConvTranspose1d(in_channels, in_channels, kernel_size=2, padding=0, stride=2, std_mul=1.0),
          Conv1dGLU(
            n_speakers,
            speaker_embed_dim,
            in_channels,
          ...
          ConvTranspose1d(in_channels, in_channels, kernel_size=2, padding=0, stride=2, std_mul=4.0),
          Conv1dGLU(
            n_speakers,
            speaker_embed_dim,
            in_channels,
    ...
    # Last 1x1 convolution
    self.convolutions.append(
      Conv1d(in_channels, out_dim, kernel_size=1, padding=0, dilation=1, std_mul=std_mul, dropout=dropout)
    )
