...
def test_conv1d_incremental():
  ...
  def __test(kernel_size, dilation, T, B, C, causual=True):
    ...
    # weight: (Cout, Cin, K)
    conv = nn.Conv1d(C, C * 2, kernel_size=kernel_size, padding=padding, dilation=dilation).eval()
    conv.weight.data.fill_(1.0)
    conv.bias.data.zero_()

    # weight: (K, Cin, Cout)
    # weight (linearized): (Cout*K, Cin)
    conv_online = Conv1d(C, C * 2, kernel_size=kernel_size, padding=padding, dilation=dilation).eval()
