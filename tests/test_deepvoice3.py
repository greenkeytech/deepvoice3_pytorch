...
def _get_model(
  n_speakers=1, speaker_embed_dim=None, force_monotonic_attention=False, use_decoder_state_for_postnet_input=False
):
  model = deepvoice3(
    n_vocab=n_vocab,
    embed_dim=256,
...
def _deepvoice3(
  ...
  encoder = Encoder(
    ...
    speaker_embed_dim=speaker_embed_dim,
    dropout=dropout,
    convolutions=[(h, 3, dilation), (h, 3, dilation), (h, 3, dilation), (h, 3, dilation), (h, 3, dilation)],
  )
...
def test_incremental_correctness():
  ...
  for model, speaker_ids in [
    ...
    mel_outputs_online, alignments, dones_online, _ = model.seq2seq.decoder.incremental_forward(
      encoder_outs, text_positions, speaker_embed=speaker_embed, test_inputs=mel_reshaped
    )
    ...
    assert np.allclose(mel_outputs_offline.cpu().data.numpy(), mel_outputs_online.cpu().data.numpy(), atol=1e-5)
...
def test_incremental_forward():
  ...
  def _plot(mel, mel_predicted, alignments):
    ...
    plt.subplot(3, 1, 2)
    plt.imshow(mel_predicted.view(-1, mel_dim).data.cpu().numpy().T, origin="lower bottom", aspect="auto", cmap="magma")
    plt.colorbar()
  ...
  mel_output_offline, alignments_offline, done = model.seq2seq.decoder(
    encoder_outs, mel_reshaped, text_positions=text_positions, frame_positions=frame_positions, lengths=input_lengths
  )
