...
def test_nyanko_basics():
  ...
  for v in [False, True]:
    model = nyanko(
      n_vocab, mel_dim=num_mels, linear_dim=num_freq, r=1, downsample_step=4, use_decoder_state_for_postnet_input=v
    )
...
def test_incremental_correctness():
  ...
  model = nyanko(n_vocab, mel_dim=mel_dim, linear_dim=513, downsample_step=4, r=r, force_monotonic_attention=False)
  model.eval()
  ...
  mel_outputs_offline, alignments_offline, done, _ = model.seq2seq.decoder(
    encoder_outs, mel_reshaped, text_positions=text_positions, frame_positions=frame_positions
  )
  ...
  assert np.allclose(mel_outputs_offline.cpu().data.numpy(), mel_outputs_online.cpu().data.numpy())
