...
def pwrap(args, shell=False):
  p = subprocess.Popen(
    args, shell=shell, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
  )
  return p
...
def extract_intermediate_features(wav_path, txt_path, keep_silences=False, full_features=False, ehmm_max_n_itr=1):
  basedir = os.getcwd()
  ...
  if do_state_align:
    ...
    if not os.path.exists("misc/scripts/alignment/phone_align/full-context-labels/full"):
      ...
      if not full_features:
        ...
        do_ehmm_lines[
          replace_line
        ] = "    $EHMMDIR/bin/ehmm ehmm/etc/ph_list.int ehmm/etc/txt.phseq.data.int 1 0 ehmm/binfeat scaledft ehmm/mod 0 0 0 %s $num_cpus\n" % str(
          max_n_itr
        )
  ...
  pe(
    "sed -i.bak -e s/MERLIN_THEANO_FLAGS=.*/MERLIN_THEANO_FLAGS='device=cpu,floatX=float32,on_unused_input=ignore'/g src/setup_env.sh",
    shell=True
  )
  os.chdir("tools")
  ...
  print(
    "Audio features in %s (and %s)" %
    (os.getcwd() + "/audio_feat", merlin_dir + "/egs/slt_arctic/s1/slt_arctic_full_data/feat")
  )
  print(
    "Text features in %s (and %s)" %
    (os.getcwd() + "/text_feat", merlin_dir + "/misc/scripts/alignment/phone_align/full-context-labels/full")
  )
  os.chdir(basedir)
...
def extract_final_features():
  ...
  if not os.path.exists(basedir + "final_acoustic_data"):
    os.symlink(os.path.abspath("experiments/my_new_voice/acoustic_model/data"), basedir + "final_acoustic_data")
  if not os.path.exists(basedir + "final_duration_data"):
    os.symlink(os.path.abspath("experiments/my_new_voice/duration_model/data"), basedir + "final_duration_data")
  os.chdir(launchdir)
...
def save_numpy_features():
  ...
  if not os.path.exists(monophone_path):
    ...
    os.symlink(
      os.path.abspath("latest_features/merlin/misc/scripts/alignment/phone_align/cmu_us_slt_arctic/lab"),
      monophone_path[:-1]
    )

  launchdir = os.getcwd()
  phone_files = {gl[:-4]: monophone_path + gl for gl in os.listdir(monophone_path) if gl[-4:] == ".lab"}
  ...
  error_files = [(i, x) for i, x in enumerate(text_ids) if x not in file_list]
  ...
  for i, idx in enumerate(order):
    ...
    # if i % 100 == 0:
    #    print(i)
    in_features, lab_frame_number = load_binary_file_frame(label_files[fid], n_ins)
    out_features, out_frame_number = load_binary_file_frame(audio_files[fid], n_outs)
    ...
    first_phoneme = next(k - 1 for k, x in enumerate(phonemes) if x != 'pau')

    last_phoneme = len(phonemes) - next(k - 1 for k, x in enumerate(phonemes[::-1]) if x != 'pau')
  ...
  for i in range(len(all_ids)):
    ...
    np.savez_compressed("latest_features/numpy_features/%s.npz" % all_ids[i], **save_dict)


def generate_merlin_wav(
  data,
  gen_dir,
  file_basename=None,  # norm_info_file,
  ...
  stream_start_index = {}
  file_extension_dict = {'mgc': '.mgc', 'bap': '.bap', 'lf0': '.lf0', 'dur': '.dur', 'cmp': '.cmp'}
  ...
  for feature_name in gen_wav_features:
    ...
    current_features = features[:, stream_start_index[feature_name]:
                                stream_start_index[feature_name] + out_dimension_dict[feature_name]]
  ...
  world_path = {'ANALYSIS': worlddir + 'analysis', 'SYNTHESIS': worlddir + 'synth'}
  ...
  if do_post_filtering:
    ...
    pe(
      '{line} | {x2x} +af > {weight}'.format(line=line, x2x=sptk_path['X2X'], weight=os.path.join(gen_dir, 'weight')),
      shell=True
    )
  ...
  pe(
    ...
    '{x2x} +fd > {f0}'.format(sopr=sptk_path['SOPR'], lf0=files['lf0'], x2x=sptk_path['X2X'], f0=files['f0']),
    shell=True
  )

  pe(
    '{sopr} -c 0 {bap} | {x2x} +fd > {ap}'.format(
      sopr=sptk_path['SOPR'], bap=files['bap'], x2x=sptk_path['X2X'], ap=files['ap']
    ),
    shell=True
  )
  ...
  pe(
    '{synworld} {fl} {sr} {f0} {sp} {ap} {wav}'.format(
      synworld=world_path['SYNTHESIS'], fl=fl, sr=sr, f0=files['f0'], sp=files['sp'], ap=files['ap'], wav=files['wav']
    ),
    shell=True
  )
...
def get_reconstructions():
  ...
  for fp in os.listdir(features_dir)[:5]:
    ...
    generate_merlin_wav(r, "latest_features/gen", file_basename=fp.split(".")[0], do_post_filtering=False)
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser(
    description=
    "Extract audio and text features using speech synthesis toolkits including SPTK, HTS, HTK, and Merlin. Special thanks to Jose Sotelo and the Edinburgh Speech Synthesis team. The text to use must not contain any parenthesis characters e.g. '(' or ')' .",
    epilog="Example usage: python extract_features.py -w wav48/p294 -t txt/p294"
  )
  parser.add_argument("--wav_dir", "-w", help="filepath for directory of wav files", required=True)
  parser.add_argument("--txt_dir", "-t", help="filepath for directory of txt files", required=True)
  parser.add_argument(
    "--keep_silences",
    "-k",
  ...
  for i in itr:
    ...
    if not os.path.exists("latest_features"):
      ...
      if not os.path.exists("latest_features/text_feat") and not os.path.exists("latest_features/audio_feat"):
        ...
        os.chdir(pdir)
        extract_intermediate_features(cur_wav_dir, cur_txt_dir, keep_silences, full_features)
    if not os.path.exists("latest_features/final_duration_data"
                         ) or not os.path.exists("latest_features/final_acoustic_data"):
    ...
    if multifolder:
      ...
      shutil.copytree("latest_features" + str(os.sep) + "numpy_features", tmp_results)
  if multifolder:
    for i in itr:
      for f in os.listdir("tmp_results_%i" % i):
        try:
          shutil.move("tmp_results_%i" % i + str(os.sep) + f, "latest_features" + str(os.sep) + "numpy_features")
        except shutil.Error:
          continue
