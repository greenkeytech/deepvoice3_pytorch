...
class _NPYDataSource(FileDataSource):
  ...
  def collect_files(self):
    ...
    self.frame_lengths = list(map(lambda l: int(l.decode("utf-8").split("|")[2]), lines))
...
def collate_fn(batch):
  ...
  b = np.array([_pad_2d(x[1], max_target_len, b_pad=b_pad) for x in batch], dtype=np.float32)
  mel_batch = torch.FloatTensor(b)

  c = np.array([_pad_2d(x[2], max_target_len, b_pad=b_pad) for x in batch], dtype=np.float32)
  y_batch = torch.FloatTensor(c)

  # text positions
  text_positions = np.array([_pad(np.arange(1, len(x[0]) + 1), max_input_len) for x in batch], dtype=np.int)
  ...
  # if b_pad > 0:
  #    s, e = s - 1, e - 1
  frame_positions = torch.arange(s, e).long().unsqueeze(0).expand(len(batch), max_decoder_target_len)

  # done flags
  done = np.array(
    [_pad(np.zeros(len(x[1]) // r // downsample_step - 1), max_decoder_target_len, constant_values=1) for x in batch]
  )
...
def save_alignment(path, attn):
  plot_alignment(attn.T, path, info="{}, {}, step={}".format(hparams.builder, time_string(), global_step))
...
def eval_model(global_step, writer, model, checkpoint_dir, ismultispeaker):
  ...
  for speaker_id in speaker_ids:
    ...
    for idx, text in enumerate(texts):
      signal, alignment, _, mel = synthesis.tts(model, text, p=0, speaker_id=speaker_id, fast=False)
      signal /= np.max(np.abs(signal))

      # Alignment
      path = join(eval_output_dir, "step{:09d}_text{}_{}_alignment.png".format(global_step, idx, speaker_str))
      ...
      # Mel
      writer.add_image(
        "(Eval) Predicted mel spectrogram text{}_{}".format(idx, speaker_str), prepare_spec_image(mel), global_step
      )

      # Audio
      path = join(eval_output_dir, "step{:09d}_text{}_{}_predicted.wav".format(global_step, idx, speaker_str))
      audio.save_wav(signal, path)

      try:
        writer.add_audio(
          "(Eval) Predicted audio signal {}_{}".format(idx, speaker_str), signal, global_step, sample_rate=fs
        )
...
def save_states(global_step, writer, mel_outputs, linear_outputs, attn, mel, y, input_lengths, checkpoint_dir=None):
  ...
  if attn is not None and attn.dim() == 4:
    for i, alignment in enumerate(attn):
      ...
      path = join(alignment_dir, "step{:09d}_layer_{}_alignment.png".format(global_step, i + 1))
      save_alignment(path, alignment)
...
def guided_attentions(input_lengths, target_lengths, max_target_len, g=0.2):
  ...
  for b in range(B):
    W[b] = guided_attention(input_lengths[b], max_input_len, target_lengths[b], max_target_len, g).T
  return W
    ...
    for step, (x, input_lengths, mel, y, positions, done, target_lengths,
      ...
      if train_seq2seq and train_postnet:
        ...
        mel_outputs, attn, done_hat, _ = model.seq2seq(
          x, mel, text_positions=text_positions, frame_positions=frame_positions, input_lengths=input_lengths
        )
        # reshape
      ...
      loss.backward()
      if clip_thresh > 0:
        grad_norm = torch.nn.utils.clip_grad_norm(model.get_trainable_parameters(), clip_thresh)
      optimizer.step()
      ...
      if train_postnet:
        ...
        writer.add_scalar("linear_binary_div_loss", float(linear_binary_div.data[0]), global_step)
      if train_seq2seq and hparams.use_guided_attention:
...
def save_checkpoint(model, optimizer, step, checkpoint_dir, epoch, train_seq2seq, train_postnet):
  if train_seq2seq and train_postnet:
    suffix = ""
    m = model
  elif train_seq2seq:
  ...
  checkpoint_path = join(checkpoint_dir, "checkpoint_step{:09d}{}.pth".format(global_step, suffix))
